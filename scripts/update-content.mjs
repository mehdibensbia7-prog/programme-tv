...
async function run() {
  ...
  try {
    ...
    console.log("news-ai.json mis à jour.");
  } catch (e) { console.error("Erreur News:", e.message); }
  ...
  try {
    if (TMDB_API_KEY) {
      ...
      const items = (data.results || []).slice(0, 6).map(m => ({
          ...
          desc: m.overview ? m.overview.slice(0, 140) + "..." : "Aucun synopsis.",
          isAI: false
      }));
      writeFileSync("data/trends-ai.json", JSON.stringify({ updatedAt: Date.now(), items }, null, 2));
      console.log("trends-ai.json mis à jour.");
    }
  } catch (e) { console.error("Erreur Trends:", e.message); }
}
