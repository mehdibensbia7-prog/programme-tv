...
async function run() {
  ...
  try {
    ...
    const items = (data.items || []).slice(0, 5).map(i => ({ title: i.title, link: i.link, desc: i.description?.replace(/<[^>]*>/g, "").slice(0, 80) + "..." }));
  ...
  try {
    ...
    const items = (data.results || []).slice(0, 6).map(m => ({ 
      ...
      link: `https://www.themoviedb.org/${m.media_type}/${m.id}`,
